module aptos_intent::fa_intent {
    ...
    /// CLI-friendly wrapper for creating a regular intent requesting tokens from APT.
    ...
    public fun create_regular_intent_from_apt(
        account: &signer,
        desired_amount: u64,
        expiry_time: u64,
    ): address {
        ...
        // Withdraw 0 APT (no tokens locked, just requesting)
    ...
    /// Accepts intent_id for testability - allows tests to specify the intent ID upfront
    public entry fun create_regular_intent_from_apt_entry(
        account: &signer,
        desired_amount: u64,
        expiry_time: u64,
        _intent_id: address,
    ) {
        // Create intent with the specified ID (for testing)
        // The intent_id parameter is for cross-chain linking purposes
        create_regular_intent_from_apt(account, desired_amount, expiry_time);
        
        // Note: The actual intent object address is generated by the framework
        // The intent_id parameter is passed for cross-chain linking but not used for object creation
        // This is intentional - we create the intent and let the caller link it via events
    }
