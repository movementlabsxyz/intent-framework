...
setup_project_root
setup_logging "fulfill-outflow-hub-intent"
cd "$PROJECT_ROOT"

log ""
log "🔓 OUTFLOW HUB INTENT FULFILLMENT"
log "==================================="
log ""
...
# Load approval signature from validation step
APPROVAL_INFO_FILE="${PROJECT_ROOT}/.test-data/outflow-approval-info.txt"

if [ ! -f "$APPROVAL_INFO_FILE" ]; then
    log_and_echo "❌ ERROR: Approval info file not found at $APPROVAL_INFO_FILE"
    log_and_echo "   Please run validate-outflow-transfer.sh first"
    exit 1
fi

# Source the file to load variables
source "$APPROVAL_INFO_FILE"

if [ -z "$APPROVAL_SIGNATURE" ]; then
    log_and_echo "❌ ERROR: APPROVAL_SIGNATURE not found in approval info file"
    exit 1
fi
...
log "   Signature Type:          ${SIGNATURE_TYPE:-Ed25519}"
...
log ""
log "   Fulfilling outflow intent on hub chain..."
...
log ""
log "🎉 OUTFLOW HUB INTENT FULFILLMENT COMPLETE!"
log "============================================"
log ""
log "✅ Steps completed successfully:"
log "   1. Solver (Bob) fulfilled hub intent with verifier signature"
log "   2. Locked tokens released to Bob on hub chain"
log "   3. Outflow intent flow complete"
log ""
log "📋 Intent Details:"
log "   Intent ID: $INTENT_ID"
log "   Hub Intent Address: $HUB_INTENT_ADDRESS"
