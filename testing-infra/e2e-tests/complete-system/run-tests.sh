...
# Update verifier.toml
VERIFIER_CONFIG="trusted-verifier/config/verifier.toml"
if [ ! -f "$VERIFIER_CONFIG" ]; then
    echo "❌ ERROR: verifier.toml not found at $VERIFIER_CONFIG"
    exit 1
fi

# Update module addresses in verifier.toml
sed -i "/\[hub_chain\]/,/\[connected_chain\]/ s|intent_module_address = .*|intent_module_address = \"0x$CHAIN1_ADDRESS\"|" "$VERIFIER_CONFIG"
sed -i "/\[connected_chain\]/,/\[verifier\]/ s|intent_module_address = .*|intent_module_address = \"0x$CHAIN2_ADDRESS\"|" "$VERIFIER_CONFIG"
sed -i "/\[connected_chain\]/,/\[verifier\]/ s|escrow_module_address = .*|escrow_module_address = \"0x$CHAIN2_ADDRESS\"|" "$VERIFIER_CONFIG"
...
if [ -n "$ALICE_CHAIN1_ADDRESS" ] && [ -n "$BOB_CHAIN1_ADDRESS" ]; then
    sed -i "/\[hub_chain\]/,/\[connected_chain\]/ s|known_accounts = .*|known_accounts = [\"$ALICE_CHAIN1_ADDRESS\", \"$BOB_CHAIN1_ADDRESS\"]|" "$VERIFIER_CONFIG"
fi

if [ -n "$ALICE_CHAIN2_ADDRESS" ]; then
    sed -i "/\[connected_chain\]/,/\[verifier\]/ s|known_accounts = .*|known_accounts = [\"$ALICE_CHAIN2_ADDRESS\"]|" "$VERIFIER_CONFIG"
fi

echo "✅ Updated verifier.toml with deployed addresses"
echo ""
...
#[path = "../../testing-infra/e2e-tests/complete-system/mod.rs"]
mod integration;
EOF
