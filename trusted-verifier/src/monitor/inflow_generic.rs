            ...
            Ok(events) => {
                for event in events {
...

                    {
                        ...
                        if !escrow_cache
                            .iter()
                            .any(|cached| cached.escrow_id == escrow_id && cached.chain_id == chain_id)
                        {
...
pub async fn validate_request_intent_fulfillment(monitor: &EventMonitor, escrow_event: &EscrowEvent) -> Result<()> {
    info!(
...
pub async fn validate_and_approve_fulfillment(monitor: &EventMonitor, fulfillment: &FulfillmentEvent) -> Result<()> {
    ...
    let (signature_bytes, timestamp) = if is_evm_escrow {
        ...
        let ecdsa_sig_bytes = monitor.crypto.create_evm_approval_signature(intent_id_hex)?;
...
pub async fn get_approval_for_escrow(monitor: &EventMonitor, escrow_id: &str) -> Option<EscrowApproval> {
