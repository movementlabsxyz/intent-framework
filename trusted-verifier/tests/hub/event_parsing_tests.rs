...
fn create_mock_oracle_limit_order_event(
    ...
    let requester_addr_opt = requester_address_connected_chain.map(|addr| {
        json!({
            "vec": [addr]
        })
...
async fn setup_mock_server_with_oracle_event(
    ...
    Mock::given(method("GET"))
        .and(path(format!("/v1/accounts/{}/transactions", account_address)))
        .respond_with(ResponseTemplate::new(200)
            .set_body_json(transactions_response))
        .mount(&mock_server)
        .await;
...
async fn test_poll_hub_events_populates_requester_address_connected_chain() {
    ...
    let requester_address_connected_chain = "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    ...
    let events = trusted_verifier::monitor::poll_hub_events(&monitor).await
    ...
    assert_eq!(event.intent_id, "0x1111111111111111111111111111111111111111111111111111111111111111");
    assert_eq!(event.reserved_solver, Some("0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb".to_string()));
